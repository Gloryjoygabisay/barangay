...
`vite.config.ts` uses `base: './'`, which keeps the build compatible with static hosting such as GitHub Pages. The deployment workflow sets `VITE_APP_VERSION` so the UI shows a unique build identifier per deploy. Browser monitoring is loaded from the static New Relic snippet in `index.html`.
